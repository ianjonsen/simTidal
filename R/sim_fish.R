...
##' @importFrom terra extract nlyr crs
...
sim_fish <-
  function(id=1,
    ...
    if (!is.null(data)) {
      ...
      if (length(grep("\"kilometre\"", crs(data$land))) == 0)
        stop("land raster projection must have units in km")
      ...
      if (!is.null(dim(mpar$coa)) & !is.null(mpar$coa)) {
        coa.test <- extract(data$land, t(mpar$coa))
    ...
    if (all(mpar$move == "bcrw", any(is.na(mpar$bearing)))) {
    ...
    if (mpar$move == "bcrw.coa" & length(mpar$bl) > 1) {
